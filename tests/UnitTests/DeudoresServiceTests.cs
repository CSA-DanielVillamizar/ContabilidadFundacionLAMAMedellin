...
public class DeudoresServiceTests
{
    ...
    {
        public Task<decimal> GetUsdCopAsync(DateOnly fecha, CancellationToken ct = default)
            => Task.FromResult(4000m); // TRM fijo para tests
    }
    ...
    private static (IDbContextFactory<AppDbContext> factory, AppDbContext db, IExchangeRateService trm) CreateInMemoryContext()
    {
        var factory = new TestDbFactory();
        var db = factory.CreateDbContext();
        var trm = new FakeExchangeRateService();
        return (factory, db, trm);
    }
    ...
    {
        // Arrange
        var (factory, db, trm) = CreateInMemoryContext();
        await using (db)
        {
            ...
            var svc = new DeudoresService(factory, trm);
